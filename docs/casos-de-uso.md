# Casos de Uso del Sistema — Top Tennis

> **VS Code:** abrí este archivo y presioná `Ctrl+Shift+V` para ver los diagramas renderizados.
>
> Documentación relacionada: [Casos de uso de negocio](casos-de-uso-negocio.md) · [Reglas de negocio](reglas-de-negocio.md) · [Modelo de datos](modelo-de-datos.md) · [Índice de código](../INDEX.md)
...
|---|---|---|
| **Cliente** | Usuario registrado que reserva canchas y paga con Yape o Efectivo | `users.rol = 'cliente'` ([Rol.php](../app/enums/Rol.php)) |
| **Administrador** | Gestiona el club: canchas, tarifas y horarios; opera el mostrador (cobros en efectivo, reservas manuales) y ve las finanzas | `users.rol = 'admin'` |
...
**Jerarquía de permisos:** el `Administrador` puede además hacer todo lo que hace un `Cliente` (reservar para sí mismo, ver su perfil, etc.). El middleware [RoleMiddleware.php](../app/Http/Middleware/RoleMiddleware.php) hace cumplir el acceso ruta por ruta.
...
- Si elige **Efectivo**, la reserva queda `pendiente` y caduca 30 minutos antes de la hora de juego si no se paga en el club.
...
## 3. Casos de uso del ADMINISTRADOR

### 3.1 Gestión del club

```mermaid
flowchart LR
    ADM((👑<br>Administrador))

    subgraph "TOP TENNIS — gestión del club"
        direction LR
...
### 3.2 Operación del mostrador

```mermaid
flowchart LR
    ADM((👑<br>Administrador))

    subgraph "TOP TENNIS — operación diaria"
        UC30([UC-30 Ver todas las reservas<br>con total de ingresos])
        UC30a([Filtrar por método de pago<br>Yape / Efectivo])
        UC31([UC-31 Confirmar pago en efectivo<br>pendiente → aprobado])
        UC32([UC-32 Crear reserva manual<br>cliente presencial o por teléfono])
        UC32a([Seleccionar cliente existente])
        UC32b([Registrar cliente nuevo al vuelo<br>solo nombre y teléfono])
        UC33([UC-33 Ver ticket de cualquier reserva])
        UC34([UC-34 Cancelar cualquier reserva<br>libera el horario])
        UC35([UC-35 Validar ticket del cliente<br>escanear QR / código TT-XXXX])
    end
    ...
    UC32 -.->|«extend»| UC32a
    UC32 -.->|«extend»| UC32b
    UC31 -.->|«include» verifica que no esté vencida| UC30
```
...
- En la reserva manual el pago se registra como `aprobado` directo (el cobro ocurre en el mostrador); si registra un cliente nuevo, el sistema genera email y contraseña internos aleatorios.
- Al poner una cancha **en mantenimiento**, las reservas `aprobadas` dentro del rango pasan a `cancelado_por_mantenimiento` y se registra el reembolso de cada una — todo en una transacción ([CanchaController.php](../app/Http/Controllers/CanchaController.php)).
- El **ingreso total** del dashboard suma `monto_pagado` (precio congelado al reservar), nunca el precio actual de la tarifa.

---

## 4. Casos de uso del SCHEDULER (procesos automáticos)
...
flowchart LR
    ...
    subgraph "TOP TENNIS — procesos automáticos"
        UC40([UC-40 Liberar no-shows<br>reservas en Efectivo vencidas])
        UC40a([Marcar reserva como anulada])
        UC40b([Devolver horario a disponible])
        UC41([UC-41 Restaurar canchas<br>cuyo mantenimiento terminó])
    end
...
2. **Modo lazy:** `Reserva::liberarVencidas()` y `Cancha::restaurarVencidas()` se ejecutan al inicio de las vistas clave (`disponibles()`, `confirmar()`, `canchas.index`).
...
### UC-31 — Confirmar pago en efectivo

| Campo | Detalle |
|---|---|
| **Actor principal** | Administrador |
| **Precondiciones** | Reserva en `Efectivo` con estado `pendiente` y no vencida |
| **Flujo principal** | 1. El cliente paga en el mostrador.<br>2. El administrador ubica la reserva en el historial.<br>3. Pulsa "Confirmar pago".<br>4. El sistema valida rol, método y estado, y pasa `estado_pago` a `aprobado` limpiando `expira_at`. |
| **Flujos alternos** | **4a.** La reserva ya caducó (`estaVencida()`) → se rechaza; el proceso de no-show ya la anuló o la anulará.<br>**4b.** Un usuario sin rol de administrador intenta la acción → redirección con "Solo el administrador puede confirmar pagos en efectivo". |
| **Postcondiciones** | Reserva `aprobado`; su monto cuenta en los ingresos del dashboard |
| **Código** | `ReservaController::confirmarPago()` |

### UC-32 — Crear reserva manual

| Campo | Detalle |
|---|---|
| **Actor principal** | Administrador |
| **Precondiciones** | Sesión de administrador; hay horarios reservables |
| **Flujo principal** | 1. El administrador abre "Reserva manual" (calendario agrupado por día y cancha).<br>2. Selecciona día → cancha → hora.<br>3. Elige cliente existente **o** registra uno nuevo (nombre + teléfono).<br>4. Registra el método de pago; el sistema crea la reserva ya `aprobado`. |
...
| **Flujo principal** | 1. El administrador abre el modal e ingresa **motivo** y **fecha/hora de fin** (obligatoria y futura).<br>2. En una transacción: las reservas `aprobadas` con juego dentro del rango pasan a `cancelado_por_mantenimiento` y se registra el reembolso de cada una (código, cliente, monto) en el log.<br>3. La cancha pasa a `en_mantenimiento` y desaparece del catálogo reservable. |
| **Flujos alternos** | **1a.** Fecha de fin en el pasado → error de validación. |
| **Postcondiciones** | Cancha fuera de servicio hasta `fin_mantenimiento`; al vencer, `restaurarVencidas()` la reactiva automáticamente (UC-41) |
| **Código** | `CanchaController::ponerMantenimiento()` / `restaurar()` |

### UC-40 — Liberar no-shows (automático)
...
## 6. Matriz Rol × Caso de uso

| Caso de uso | Cliente | Admin | Scheduler |
|---|:---:|:---:|:---:|
| Registrarse / iniciar sesión | ✅ | ✅ | — |
| Ver horarios disponibles | ✅ | ✅ | — |
| Reservar y pagar (Yape/Efectivo) | ✅ | ✅ | — |
| Ver/descargar ticket QR | solo suyos | todos | — |
| Ver historial de reservas | solo suyas | todas + total | — |
| Cancelar reserva | solo suyas | todas | — |
...
> El control se hace cumplir en dos niveles: **rutas** ([web.php](../routes/web.php), middleware `role:admin`) y **registros** (`autorizar()` para que un cliente no acceda a reservas ajenas por URL).
