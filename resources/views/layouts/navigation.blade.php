        ...
        <div class="flex justify-between h-16">
            ...
            <div class="flex items-center gap-8">
                ...
                <a href="{{ route('dashboard') }}" class="flex items-center gap-2.5 shrink-0">
                    ...
                    <span class="font-extrabold text-sm tracking-widest uppercase text-gray-900 hidden sm:block">
                        Top Tennis Digital
                    </span>
                </a>
            ...
            <div class="hidden sm:flex items-center gap-3">
                ...
                @endphp
                <span class="text-xs font-semibold px-2.5 py-1 rounded-full {{ $rolColor }}">
                    {{ ucfirst($rol) }}
                </span>

                <div x-data="{ open: false }" class="relative">
                    <button @click="open = !open"
                            class="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-semibold text-gray-700 hover:bg-gray-50 transition-colors">
    ...
    <div :class="{'block': open, 'hidden': !open}" class="hidden sm:hidden border-t border-gray-100">
        ...
        <div class="px-4 py-3 border-t border-gray-100">
            ...
            <div class="mt-2 space-y-1">
                <a href="{{ route('profile.edit') }}" class="block px-3 py-2 text-sm text-gray-600 hover:bg-gray-50 rounded-lg">Mi perfil</a>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="block w-full text-left px-3 py-2 text-sm text-red-600 hover:bg-red-50 rounded-lg">
                        Cerrar sesión
                    </button>
                </form>
            </div>
